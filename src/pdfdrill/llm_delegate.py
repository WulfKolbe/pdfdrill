...
def _instruction_block(tasks: list[LLMTask], llm_dir: Path) -> str:
    ...
    lines = [
        ...
        f"No API keys present. {len(tasks)} LLM task(s) deferred to you, the agent",
        f"running pdfdrill ({n_vis} vision, {n_bib} bibliographic).",
        "",
        ...
        "     (you can see it directly); otherwise just read the prompt.",
        "  2. Do exactly what `prompt` asks, honouring `schema`.",
