...
#: 616A — `\(` … `\)` inside a listing body. MathPix emits the listing with
#: `[mathescape=true]` and then uses `\(`…`\)` for the inline maths, which is
#: not what `listings` mathescape reads — it reads `$`. Measured over 1,369
#: documents: 464 rows carry an lstlisting and 407 of them (88%) contain `\(`,
#: so this is the common case and not an oddity.
_LST_MATH = re.compile(r"\\\((.+?)\\\)", re.S)


def listing_cell(body: str, limit: int = 1200) -> str:
    r"""A listing body as escaped monospace, with its `\(…\)` spans SET.

    The substitution the task asks for is `\(` -> `$`, and it cannot be made
    literally here: this cell is escaped monospace, not the `listings`
    environment (no report.tex emits that environment — measured, 0 of them),
    so a `$` would print as a dollar sign rather than open maths. The spans are
    therefore split out and set as real inline maths, which is what `$` would
    have meant.

    THE GUARD. A body that ALREADY contains a `$` is left entirely alone. In
    `listings` with mathescape the `$` is itself the delimiter, so a body
    carrying one is either already escaping or contains a literal dollar, and
    substituting around it would produce an unbalanced escape. Corpus-wide
    exactly one row is in this state — the Geothermal handbook's DIA_0021,
    whose body holds `Cp(Brine\$,T=T_avg_fluid,...)` — so the guard costs one
    row of maths and prevents one broken cell.
    """
    body = (body or "")[:limit]
    if not body:
        return ""
    if "$" in body:
        return "{\\ttfamily\\tiny %s}" % esc_text(body)
    out, last = [], 0
    for m in _LST_MATH.finditer(body):
        out.append("{\\ttfamily\\tiny %s}" % esc_text(body[last:m.start()]))
        safe = renderable(m.group(1))
        out.append("$%s$" % safe if safe
                   else "{\\ttfamily\\tiny %s}" % esc_text(m.group(0)))
        last = m.end()
    out.append("{\\ttfamily\\tiny %s}" % esc_text(body[last:]))
    return "".join(out)
...
#: 605 — the cell rect, one row per report row, in PDF user space (bp).
ROWS_MANIFEST = "pdfdrill-rows.json"

#: The table rule the builder draws. The consumer needs it because a savepos
...
#: order, is the only form that yields all three. Verified under xelatex.
#: 605 — per-build state for the cell rect. Reset by `build_report`; `row()`
...
def _cellrect_marks(title: str) -> tuple:
    """(open, close) savepos marks for one row, or ("", "").

    The label is a COUNTER, not the identifier: identifiers carry `.` and `_`
    and are up to 60 characters, and a zref label is a control-sequence key.
    The mapping back to the identifier is written into the manifest.
    """
    if not _CELLRECT["on"]:
        return "", ""
    _CELLRECT["n"] += 1
...
%% 616B — A PACKAGE THAT SILENTLY IS NOT THERE IS WORSE THAN ONE THAT FAILS.
%% The else branch used to be empty, so a missing listings.sty produced a
%% report that differed and said nothing. It now warns in the log and defines
%% a marker the reader can see. Note that NO report.tex currently emits the
%% lstlisting environment — listing bodies are set as escaped monospace by
%% `listing_cell` — so this branch has never yet changed an output.
\IfFileExists{listings.sty}{\usepackage{listings}}{%%
  \PackageWarningNoLine{pdfdrill}{listings.sty is NOT installed; listing rows
    are set as plain monospace}%%
  \providecommand{\pdfdrillnolistings}{\textbf{[listings.sty absent]}}}
...
%% 665 — [no-math]: \setmainfont ALONE makes fontspec silently repoint every
%% math alphabet at the main text font ("Adjusting the maths setup", its own
%% log line) — \symoperators (the font behind \sin, \cos, \tan, \log, \lim,
%% \max, \det, every amsopn name and every \DeclareMathOperator) moves from
%% Computer Modern to DejaVu Serif, along with \mathrm/\mathbf/\mathit/
%% \mathsf/\mathtt. Nobody asked for that: \setmainfont is here only for the
%% Source column's raw-Unicode coverage (the comment above). Under xelatex,
%% DejaVu Serif's "operators" instance, re-measured across longtable's
%% multi-pass box measurement, renders the THREE-LETTER STRING "cos" as bold
%% caps (glyph corruption at the xdvipdfmx/font-subsetting level — the PDF's
%% ToUnicode CMap still says "cos", only the drawn glyphs are wrong); every
%% other operator name was tested and is unaffected BY LUCK OF THE STRING,
%% not by any protection — proved by \DeclareMathOperator{\mycos}{cos},
%% which breaks the same way under a macro that has nothing to do with
%% \cos. \cos itself is untouched: the fix is not to it, or to any operator
%% macro, but to stop fontspec from touching math fonts at all, restoring
%% the plain amsmath/amssymb Computer Modern math this preamble was already
%% written against (out/665, 0902.0431 EQ0368 and 7 more).
...
def page_height_bp(pdf: Path) -> float:
    """The paper height in bp, from the PDF itself rather than the geometry
    options — 605's consumer needs the number the page actually has."""
    import subprocess
    try:
...
def compile_fixpoint(tex_path: Path, max_iter: int = 40):
    ...
    try:
        ...
        # 605 — THE RECTS ARE READ HERE, NOT PUBLISHED. The .aux must not be
        # copied beside the document: pass N writes it and pass N+1 reads it,
        # so a shared one resolves a build's cross-references against another
        # build's numbering (297). It is parsed while the private directory
        # still exists and only the RESULT leaves.
        if _CELLRECT["on"]:
            _CELLRECT["rects"] = cellrect_from_aux(
                src_tex.with_suffix(".aux"), 0.0)
...
def build_report(tiddlers_path: Path, out: Path | None = None,
    ...
    out_parts[0] = preamble(**{"bbdigits": MATHBB_DIGITS,
                               ...
                               "cellrect": CELLRECT_PREAMBLE if cellrect else "",
...
def build_refined_report(changes_path: Path, out: Path | None = None,
    ...
    parts[0] = PREAMBLE % {"cellrect": "", "bbdigits": MATHBB_DIGITS,
