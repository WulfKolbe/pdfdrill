...
FORM_PREAMBLE = r"""\usepackage{hyperref}
\special{pdf:put @catalog <</NeedAppearances true>>}
\definecolor{inkMissed}{RGB}{220,30,30}
\definecolor{inkStraddle}{RGB}{230,150,20}
\definecolor{inkOverlap}{RGB}{40,90,220}
\definecolor{inkInside}{RGB}{40,160,40}
\definecolor{inkNoise}{RGB}{120,120,120}
\definecolor{inkClean}{RGB}{160,160,160}
\newcommand{\inkbullet}[1]{{\footnotesize\textcolor{#1}{$\bullet$}}}
\newcommand{\inkfield}[1]{\TextField[name={\detokenize{ink.#1}},width=2.4em,
  height=1.15em,borderwidth=0.6pt,bordercolor=black,backgroundcolor=white,
  charsize=5pt,align=1,readonly=true,value={\texttt{\tiny .}}]{}}
"""
...
def residual_colour(conf) -> str:
    """Compile-time colour for the residual bullet.

    inkdrill's residual class is not known at build time — it is measured from
    the finished PDF and written into the form field afterwards. So the bullet
    starts NEUTRAL (inkClean) and the field carries the measured code.

    Colouring it from confidence instead would be worse than leaving it grey:
    it would look like a residual reading while actually restating the square
    beside it, and out/145 showed the two do not agree — three unrenderable
    rows sit above confidence 0.5 and the only compile failure is at 0.914.
    """
    return "inkClean"
...
def row(title, latex, page, extra="", image=None, punct="", conf="",
        form=False, residual="inkClean") -> str:
    ...
    cell = conf_cell(conf)
    if form:
        # square (confidence) + bullet (residual class) + the update channel
        cell = "%s\\hspace{0.6em}\\inkbullet{%s}\\hspace{0.5em}\\inkfield{%s}" % (
            cell, residual, title)
...
def build_report(tiddlers_path: Path, out: Path | None = None,
                 ...
                 types: "set[str] | None" = None, form: bool = False) -> dict:
