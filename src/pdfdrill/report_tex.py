...
def row(title, latex, page, extra="", image=None, punct="", conf="",
    ...
    cell = conf_cell(conf)
    if form:
        # 148: the code is SELECTABLE TEXT in the cell as well as the field's
        # value. A form field's text is an appearance stream — pdftotext does
        # not see it, a reader cannot drag-select it, and grep over the PDF
        # misses it. So the code goes in twice, deliberately: as type for a
        # human to copy, and in the field as the update channel a re-run
        # rewrites. They are the same string, produced from the same map.
        txt = ("\\,\\texttt{\\tiny %s}" % esc_text(code)) if code else ""
        cell = ("%s\\hspace{0.6em}\\inkbullet{%s}%s\\hspace{0.5em}"
                "\\inkfield{%s}") % (cell, residual, txt, title)
