...
| `pdfdrill snip <pdf> [--image IMAGE] [--page PAGE] [--rect RECT] [--limit LIMIT] [--force] [--gemma] [--provider PROVIDER]` | OCR each equation/table crop → competing column. Default MathPix Snip (/v3/text); --gemma routes it through the Gemma-4 vision model on Novita.ai (cheap image→LaTeX tables, needs NOVITA_API_KEY). --limit N _(network)_ |
