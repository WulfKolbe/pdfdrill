...
# ------------------------------------------------------------ 653 `corpusstatus`
#
# One row per document across the WHOLE library — the index 652's own
# per-document status.html has always lacked, generated by hand several
# times before this. Read-only across the corpus: no document is rebuilt,
# no per-document lock is taken (this reads what commands that DO hold the
# lock already wrote), and the one file this DOES write — `index.html` at
# the library root — is not one document's file, so it needs no lock either
# (a single-writer race on the index is a repo-maintenance nuisance, not a
# document clobbered mid-build).
#
# WHY THERE IS NO SECOND DICT BUILDER. 652 never factored its per-document
# `data` dict into one function — `cmd_status` builds it inline from several
# smaller read-only gatherers (`_status_conserve_ledger`, `_status_budget_
# notes`, `_status_update_record`, `_list_artifacts`). The one of those that
# matters here — the conservation/ledger numbers, the expensive part — IS
# already its own function and is called directly, unchanged, below. This
# is the reuse the brief asked for; there was no single "the dict builder"
# to call, so none was invented — a second one would drift from the first,
# which is the exact failure this codebase keeps rediscovering (out/653.txt
# says so).
#
# SCALE. The library holds ~1,362 models (656's own count) and loading every
# one's full `Document` into ONE process to run `conserve()`/`ledger.
# materialize()` is the same cost `status --html` already pays, times 1,362.
# Two guards keep a corpus-wide walk from failing outright rather than
# finishing slowly:
#   - a hard size guard (`_CORPUS_MODEL_SIZE_GUARD`) skips a model whose file
#     alone is large enough to risk exhausting memory when parsed — 656 hit
#     exactly this shape (one model at 4.96 GB, the next-largest at 149 MB)
#     and skipped it by size rather than let it blow up a corpus-wide scan;
#     the row says why, it does not silently vanish.
#   - `--only-published` reads the small named set in
#     `<library>/out/documents.json` (publishcheck.py's "the published set")
#     instead of walking the whole library, so checking just the published
#     subset stays cheap no matter how large the library grows.
#
# THE CWD TRAP. 652's own CONCERNS #1: a MODEL_BUILT content-hash proof
# records each input path AS GIVEN AT BUILD TIME — almost always relative
# ("penev_A.lines.json"), because `pdfdrill model` is normally run with the
# document's own folder as cwd. `proofs.verify()` re-hashes that recorded
# string from THIS PROCESS's cwd, so calling `_status_conserve_ledger` from
# wherever `corpusstatus` itself was invoked (typically the repo root, not
# any one document's folder) would re-hash a nonexistent relative path and
# read every genuinely fresh model as stale. 652 sidestepped this by hand,
# running its own evidence from each document's folder; a corpus walk visits
# hundreds of folders, so getting this wrong is not one row's problem, it
# would be nearly every row's. The fix is the same one, mechanised: chdir
# into the document's own folder for the one call that touches the model,
# then chdir back — never left as a global side effect of this command.
_CORPUS_MODEL_SIZE_GUARD = 500_000_000   # bytes; 656's outlier was 4.96 GB,
                                          # its next-largest was 149 MB


def _corpus_model_date(sc: "Sidecar", model_path: Path) -> "str | None":
    """When the model now on disk was (last) built. The transition log's own
    'to MODEL_BUILT' entry if one was recorded (`cmd_model`'s own
    `log_transition` call), else the model file's mtime — a model built by a
    path that never logs the transition (e.g. `add`) still gets a date."""
    for tr in reversed(sc.transitions):
        if tr.get("to") == MODEL_BUILT:
            ts = tr.get("ts")
            if ts:
                return ts
    try:
        ...
        return _dt.datetime.fromtimestamp(
            model_path.stat().st_mtime, tz=_dt.timezone.utc
        ).strftime("%Y-%m-%dT%H:%M:%SZ")
    except OSError:
        return None


def _corpus_gold_bib(sc: "Sidecar") -> dict:
    """{present, count} — the author's GOLD `.bbl`/`.bib` (`cmd_bibsource`,
    `BIBSOURCE_BUILT`), NOT the heuristic/web bibliography (`bibliography`/
    `bibfetch`) and NOT bare embedded `BIBTEX_KNOWN` metadata — those are
    different facts about a different thing."""
    present = sc.has(BIBSOURCE_BUILT)
    count = (sc.get_evidence("bibsource_references", 0) or 0) if present else 0
    return {"present": present, "count": count}


def _corpus_artefacts(blob_dir: Path) -> dict:
    """{present, total, missing} over `reports.gate.PUBLISHED_FILES` — the
    five evidence/residuals PDFs (632/633) — present in this doc's own
    folder. A stat/existence check only, same contract as `check_artifact`."""
    from .reports.gate import PUBLISHED_FILES
    present = [f for f in PUBLISHED_FILES if (blob_dir / f).is_file()]
    return {"present": len(present), "total": len(PUBLISHED_FILES),
            "missing": [f for f in PUBLISHED_FILES if f not in present]}


def _href_from_library(library: Path, p: Path) -> str:
    """`p` as a link relative to the library root, where `index.html` lives —
    robust to the legacy layout (`blob_dir` one level under a doc folder that
    is itself one level under `library`), unlike `_artref` (relative to
    `blob_dir.parent`, built for the drillui bridge, a different consumer)."""
    import os as _os
    try:
        return str(p.relative_to(library))
    except ValueError:
        return _os.path.relpath(str(p), str(library))


def _corpus_status_row(pdf: Path, blob_dir: Path, library: Path) -> dict:
    """One read-only row for `corpusstatus` (653).

    Never calls `cmd_model` or any other builder — a document with no model,
    a stale one, or one too large for this corpus-wide scan gets a row that
    SAYS SO (`conserve.available = False` + `reason`), the same posture
    `_status_conserve_ledger` (652) already holds for `status --html`, which
    this reuses unchanged rather than re-deriving the same numbers a second
    way (see the module note above)."""
    sc = Sidecar(pdf)
    bibkey = resolve_bibkey(pdf, None, sc)
    model_path = _model_path(sc)
    model_built = sc.has(MODEL_BUILT) and model_path.exists()

    if not model_built:
        conserve_data = {"available": False,
                         "reason": "no model on disk yet — run `pdfdrill model`"}
    elif model_path.stat().st_size > _CORPUS_MODEL_SIZE_GUARD:
        size_gb = model_path.stat().st_size / 1e9
        conserve_data = {
            "available": False,
            "reason": f"model is {size_gb:.2f} GB — skipped by the corpus "
                      f"size guard ({_CORPUS_MODEL_SIZE_GUARD / 1e6:.0f} MB); "
                      f"run `pdfdrill status --html {pdf.name}` on it "
                      f"directly instead"}
    else:
        # The CWD trap (module note above): re-home the process for the one
        # call that checks/loads the model, then always restore it, even on
        # an exception from inside the gatherer.
        import os as _os
        prior_cwd = _os.getcwd()
        try:
            _os.chdir(pdf.parent)
            conserve_data = _status_conserve_ledger(pdf, sc)
        finally:
            _os.chdir(prior_cwd)

    status_html = blob_dir / f"{bibkey}.status.html"
    return {
        "bibkey": bibkey,
        "pdf_name": pdf.name,
        "folder": _href_from_library(library, blob_dir),
        "model": {"built": model_built,
                  "date": (_corpus_model_date(sc, model_path)
                           if model_built else None)},
        "gold_bib": _corpus_gold_bib(sc),
        "conserve": conserve_data,
        "artefacts": _corpus_artefacts(blob_dir),
        "status_href": (_href_from_library(library, status_html)
                        if status_html.is_file() else None),
    }
...
def corpus_status_rows(library: "str | Path", only_published: bool = False) -> dict:
    ...
    see its own docstring for why this, and not a bare folder-name scan, is
...
def _corpus_row_html(r: dict) -> str:
    """One `<tr>` for `corpusstatus`'s index.html. `data-sort` attributes carry
    ...
    model = r["model"]
    if model["built"]:
        model_cell = (f'<td data-sort="{_esc(model["date"] or "")}">yes'
                     + (f' ({_esc((model["date"] or "")[:10])})'
                        if model["date"] else "") + '</td>')
    else:
        model_cell = '<td class="dim" data-sort="">no</td>'

    gb = r["gold_bib"]
    gb_cell = (f'<td data-sort="{gb["count"]:08d}">'
              + (f'yes ({gb["count"]})' if gb["present"] else "no") + '</td>')

    art = r["artefacts"]
    art_cell = (f'<td data-sort="{art["present"]:02d}">'
               f'{art["present"]}/{art["total"]}</td>')

    link_cell = (f'<td><a href="{_esc(r["status_href"])}">status</a></td>'
                if r["status_href"] else
                '<td class="dim">not generated</td>')

    return (f'<tr><td data-sort="{_esc(r["bibkey"])}">{_esc(r["bibkey"])}</td>'
           f'<td data-sort="{_esc(r["folder"])}">{_esc(r["folder"])}</td>'
           f'{model_cell}{gb_cell}{verdict_cell}{unclaimed_cell}'
...
def _render_corpus_index_html(data: dict) -> str:
    ...
    head = "".join(f'<th data-col="{i}">{_esc(c)}</th>'
                   for i, c in enumerate(cols))
    return f"""<title>PDFDRILL corpus status ({len(rows)}{_esc(subset)})</title>
...
<footer>Generated by <code>pdfdrill corpusstatus</code> — read-only, no
network, no model rebuild. Click a column header to sort.</footer>
<script>
(function() {{
  ...
  ths.forEach(function(th, idx) {{
    th.addEventListener('click', function() {{
      var asc = !(state.col === idx && state.asc);
...
def _write_corpus_index_html(library: Path, data: dict) -> Path:
    """The one write `corpusstatus` makes. Deliberately NOT `cmd_`-prefixed —
    same convention `_write_status_html` (652) and `_inkreport_chain` already
    use, so `test_doclock.py`'s AST scan (which only walks top-level `cmd_*`
    defs) never sees this literal write. No `@_writes` here either: unlike a
    per-document write, `index.html` is not one document's file, so there is
    no single bibkey to key a lock on, and the brief says so explicitly
    (item 2) — this command takes no per-document lock either, since it only
    reads what the locked writers already wrote.

    Written as BYTES with `errors="replace"` — not `write_text(encoding=
    "utf-8")` — because a corpus-wide walk hits filenames `write_text` cannot
    encode: `taskout._write`'s own docstring names 18 library folders with
    CP1252 bytes that arrive as lone surrogates (`\\udcXX`), and this command
    hit exactly that live on its first real corpus run (`UnicodeEncodeError`
    on one folder's own name, surfaced only by walking every folder — no
    single-document command had ever needed to encode that name into one
    shared file before). A byte replaced in one row's link text is a much
    smaller loss than the whole corpus index failing to write at all."""
    out_path = Path(library) / "index.html"
    html = _render_corpus_index_html(data)
    out_path.write_bytes(html.encode("utf-8", "replace"))
    return out_path


def cmd_corpusstatus(library: "str | Path | None" = None,
                     only_published: bool = False) -> str:
    """One status page across the WHOLE library — 653.

    Walks every document `sidecar.iter_documents` finds under `library`
    (default `config.library_root()`) — or, with `--only-published`, just the
    named set in `<library>/out/documents.json` — and writes `index.html` at
    the library root: one row per document (model built + date, gold
    bibliography + count, the 656 conservation verdict, unclaimed anchors,
    how many of the five 632/633 evidence/residuals files are present), each
    linking to that document's own `status --html` page (652).

    Read-only: never runs `pdfdrill model` (a stale/absent/oversized model
    gets a row that says so, not a rebuild), and takes no document lock (it
    only reads what a locked writer already produced) — see the module note
    above `_CORPUS_MODEL_SIZE_GUARD` for the scale/CWD reasoning.
    """
    from . import config as cfg
    lib = Path(library).expanduser() if library else cfg.library_root()
    if not lib.is_dir():
        return f"No such library directory: {lib}"

    result = corpus_status_rows(lib, only_published=only_published)
    if result.get("error"):
        return f"corpusstatus: {result['error']}"
