...
| `pdfdrill mathpix <pdf>` | Download MathPix OCR (`lines.json`, `md`, `tex.zip`) next to the PDF; runs when asked (arXiv gets a one-line free-route note AFTER the work); outputs on disk are reused — delete them to re-OCR. `lines.json` is the input to the LaTeX-vs-image comparison pipeline. |
...
- **Math equations, MathPix-KEYLESS (no CDN crops):** `vision` has nothing to
  ...
  **ESCAPE LADDER — when a delegated page image is hard to read (do NOT drift to
  ...
  a geometry-bearing model via `mathpix` or `ocr` → `model --force` →
...
| `pdfdrill relocate <paths> [--apply] [--library] [--no-prefix]` | Bring a library to the canonical layout, in two migrations. (1) scattered → folder: <library>/<stem>/ holding the PDF + every X.* sibling + the flattened X.pdf.drill/ blobs (X.pdf.drill.json → X.drill.json). (2) id → host.id: <library>/1702.0234/ becomes vixra.1702.0234/, the files inside renamed with it and pdfdrill-downloads.json repointed — a bare id names a viXra e-print and a pre-2015 arXiv paper equally well (805/806). The archive comes from the id SHAPE first (arithmetic), then the download URL, then the sidecar last (it is derived, and nine sidecars here name the wrong archive); unprovable ids are skipped, never guessed. --no-prefix runs only (1). Dry-run by default; --apply moves. Collision-safe + idempotent. |
| `pdfdrill artifacts <pdf> [--all] [--json]` | List the openable files in the doc's drill folder (report.html, the extracted <bibkey>.md, tiddlers/semantic/llm *.json/*.txt, SVGs) with paths — clickable in the drillui Outputs panel. Giant model JSON hidden unless --all. ALSO lists the projections NOT generated yet, each with the command that makes it: what is missing is as much a fact about the document as what is there, and it is the half a caller cannot learn by looking. --json for both lists machine-readable, which is what the drillui panel and an agent driving a reading list read. (`status` also lists them.) |
| `pdfdrill size <pdf>` | File size, page count, producer |
| `pdfdrill profile <pdf> [--json JSON]` | What is ON each page, and on what evidence — the triage layer (781o). Counting is the wrong instrument for a mixed corpus: pdftc_900k_1018.pdf carries 10,368 monospace glyphs and 170 rows of listing, so counting glyphs calls it code and counting rows calls it prose, and both are wrong about the same page — the monospace is mostly INLINE. Reports properties with their measurement (listing, inline-code, equation, inline-math, frame, coloured-fill, coloured-frame, diagram, rotated-text, no-text-layer, invisible-text), per page and rolled up, and writes <stem>.profile.json. A property that is not established is ABSENT, not false — a guess is what sends a file to a paid API. It PRICES the decision: over that 132-page manual the listings live on 37 pages, so a paid pass bills 132 for content on 37, which is what route/mathpix can consult before anyone spends a key. Free, keyless, offline; reads with pdf2mmd's glyph model ($PDF2MMD_HOME, default ~/pdf2mmd) because that model is on a patched pdfminer and is not importable here. |
...
| `pdfdrill corpusstatus [--library LIBRARY] [--only-published]` | One status page across the WHOLE library (653): walks every document under --library (default the configured library_root; --only-published reads the small named set in <library>/out/documents.json instead) and writes index.html at the library root — one row per document (model built + date, gold bibliography + count, the 656 conservation verdict, unclaimed anchors, how many of the five 632/633 evidence/residuals files are present), each linking to that document's own `status --html` page (652). Read-only: never builds a model, takes no per-document lock, and skips a model too large to parse safely in a corpus-wide scan (a stated size guard) rather than risk exhausting memory on it. |
| `pdfdrill pdfinfo <pdf>` | Full PdfInfo struct (title/author/dates/flags) |
...
| `pdfdrill fonts_layer <pdf> [--force]` | Structured per-font records (pdffonts) |
| `pdfdrill images <pdf>` | Image rectangles + metadata (pdfplumber + pdfimages -list) |
...
| `pdfdrill model <pdf> [--no-source] [--bibkey BIBKEY] [--ledger]` | Build unified docmodel from lines.json. No lines.json → MathPix (if key) → arXiv LaTeX source (arXiv) → born-digital text layer (pdfplumber, FREE/fast) → tesseract OCR (scans only). --bibkey KEY sets the tiddler prefix (persisted) |
...
| `pdfdrill cdncrops <pdf>` | The scan-crop LAYER: every EQ tiddler's MathPix crop fetched into report-crops/ (left-trimmed, cached), and every TAB tiddler's crop RENDERED there from the PDF — MathPix records no crop uri for a table, so 0 of 351 table rows across the published set had a scan until 461. Declared as a reporttex dependency so steps/--ensure can see it. A keyless model (no CDN uris) still gets its table crops; real math crops need `mathpix` first. _(network)_ |
...
| `pdfdrill generations <pdf> [--written]` | 575's guard. Reports how many distinct BUILD GENERATIONS a set of documents spans — the commit sha, dirty flag and pdfdrill version each model recorded when it was built — and refuses to reduce them to one number when there is more than one. 574 measured what that costs: seven of twenty-one models were weeks older than the rest, and the section total summed across them, 1458, was 34%% below the truth with no defect anywhere in the detection code. Models built before 575 carry no stamp and are counted as their own unknown generation rather than folded into the majority. Takes any number of PDFs, document folders or model paths. |
| `pdfdrill breport <pdf> [--paper PAPER] [--portrait] [--no-compile] [--no-images] [--pages PAGES]` | ALIAS of residuals --pdf. B (B.tex/B.pdf): EVERY row of the document — display equations, inline formulas, tables and image regions — in three columns and no more: the LaTeX MathPix returned, that LaTeX rendered through THIS DOCUMENT'S OWN PREAMBLE, and the picture it was read from. Rows carry their A-state (corrected / unresolved / flagged / doubted) in place, so report.pdf and B cannot disagree about a row. Inline formulas inherit the page, confidence and region of the LINE they were printed in — the only ones they have — and the section says so. B is LaTeX and not HTML because KaTeX renders with no preamble: 482/484 measured \bm at 327 occurrences over 7 documents, \Perp defined by no package across 10, and 11,088 of 11,624 undefined occurrences being the source document's own macros, so an HTML rendering cannot be evidence in either direction. |
| `pdfdrill reporttex <pdf> [--paper PAPER] [--portrait] [--compile] [--no-images] [--formulas FORMULAS] [--findings] [--min-conf MIN_CONF] [--max-conf MAX_CONF] [--types TYPES] [--form] [--render-regions] [--prefer-refined PREFER_REFINED] [--refined] [--no-legend] [--ink INK] [--pages PAGES]` | SUPERSEDED for reading by evidence --all-kinds --pdf; kept as the ink chain's measure build. LaTeX formula report (report.tex): one boxed longtable row per EQ/FO/TAB — identifier, page, escaped source, rendered math, and the MathPix scan crop at pixel-exact original size (px2mm derived from pdfinfo + lines.json); the tex.zip unrecovered image regions (TikZ/table/failed-math candidates) close the report. --compile runs the xelatex fixpoint (malformed OCR rows demoted to source-only, never failing the document). |
...
| `pdfdrill inktables <pdf> [--ink INK] [--page PAGE]` | Adjudicate table structure between the model's tables and the ink - a ruled table's HOLES are its cells (52 holes = 13x4), booktabs has none, and the hole count travels so the discriminator is visible |
...
| `pdfdrill rename <pdf> <name> [--bibkey BIBKEY] [--dry-run]` | Rename a drilled FOLDER (and optionally its bibkey namespace) WITHOUT a rebuild. Moves the directory and its <stem>.* files, retargets the PATH and IDENTIFIER fields inside model / sidecar / tiddlers / report.tex, moves the bibkey-named crops and OKF bundle, and records the prior names in meta.folder_history and meta.bibkey_history. Never substitutes the old name as a string: an arXiv folder is named for the arXiv id, which is printed on page 1 and therefore sits in the OCR text, the PDF title and the derived BibTeX. Never rebuilds, so translations and in-place enrichments survive. |
...
| `pdfdrill inkreport <pdf> [--profile PROFILE] [--preflight-only] [--timeout TIMEOUT] [--pages PAGES] [--full] [--accept-stale-geometry]` | ALIAS of residuals --measure --pdf. THE ink chain, one command, and the only supported way to produce an ink-updated report: preflight (reads disk, at most ONE crop probe, spends nothing) -> build --no-legend (phase=measure) -> inkdrill measure -> inkconvert --force -> build with legend+ink (phase=reading) -> inspect -> publishready. Each step verifies its own output and the run stops at the first failure, naming it. `--no-legend` is INTERNAL: forgetting it stamped phase=reading and got the measurement refused five steps later (400). Refuses rather than proceeds if a paid step (mathpix/cdncrops) would be needed. Resumable: a report.ink.json newer than the measure-phase stamp skips to the reading build. PGM intermediates are deleted per page. |
...
| `pdfdrill evidence <pdf> [--kind KIND] [--all-kinds] [--pdf] [--no-images] [--paper PAPER] [--portrait] [--no-compile] [--budget-mb BUDGET_MB] [--marks MARKS]` | EVIDENCE listing, one file per kind (evidence-<kind>.html, or .pdf with --pdf): every display equation, inline formula, table or image region of the document in six columns — Identifier, Page, Conf., LaTeX source, Rendered, Image — unbounded, for lookup rather than reading. Inline formulas show their HOST LINE's page, confidence and picture, and the file says so. The PDF renders through the document's own preamble, which is the rendering that counts as evidence; the HTML is KaTeX and carries the caveat. No ink, no legend, no findings: those are `residuals`. |
| `pdfdrill residuals <pdf> [--pdf] [--measure] [--conf CONF] [--pages PAGES] [--no-images] [--paper PAPER] [--portrait] [--no-compile] [--timeout TIMEOUT] [--budget-mb BUDGET_MB] [--accept-stale-geometry]` | THE ACTION LIST (residuals.html, or .pdf with --pdf): only rows with something open, worst first — Corrected (both readings against one scan), Unresolved (does not render through the document's own preamble), Flagged (ink differs, banded on the component delta, the rest stated as a count), Low confidence (MathPix confidence under --conf, default 0.1), Doubted but correct. Six columns plus the ink bullet and code, legend on every page, bounded at 10 pages. The header states WHEN the equations were measured and against which model. --measure runs the ink chain first (the existing inkreport chain; no measurement of inline formulas exists yet). Reads report.ink.json as it stands. |
| `pdfdrill regionink <pdf> [--force]` | Measure the report's two image columns with inkdrill (284). Renders each image-table page at 300 and 600 dpi, runs `inkdrill compare` — whose default is the LAST TWO columns, which is why Rendered and Scan are last — joins the result to the build's own row manifest and writes report.regions.ink.json BESIDE report.ink.json. Never merged into it: a row with no LaTeX shows the scan twice and its distance is a self-comparison, not agreement between two sources, so every record carries `duplicated` taken from the manifest rather than inferred from pixels. The join is positional and its count is ASSERTED — a mismatch refuses rather than zipping. |
| `pdfdrill marks <pdf> [--bibkey BIBKEY] [--key KEY] [--build] [--marks-root MARKS_ROOT] [--library LIBRARY]` | inkdrill's formula marks (`tools/formulamarks.py marks`) as a command (673): run it, validate what comes back, write marks.json BESIDE the document, report. Stdout is the only thing parsed (progress is on stderr and is discarded); inkdrill's own `refused` field is reported as a refusal and never as success; `reading_changed` rows are always in the first lines of the report. The key inkdrill actually uses is the LIBRARY FOLDER NAME, tried before pdfdrill's own bibkey/site slug (they differ for every Z-Library book) — the report always says which one matched, and refuses rather than guessing when both resolve and disagree. Stops at the marks file by default; --build also draws the marks onto the evidence PDF's FORMULA kind only (marks never apply to the other three) in the same call. |
...
| `pdfdrill spoken <pdf> [--n N] [--id ID] [--out OUT] [--json] [--fallback FALLBACK] [--print] [--footnotes FOOTNOTES] [--cites CITES]` | the LLM INPUT text — prose in flow order with each math marker replaced by its spoken form; formulas with no spoken yet are shown visibly, not dropped |
...
Attestation token — the LAST line of this SKILL. If you can read this, you read the whole file. Run `pdfdrill preflight --ack DRILL-41e01316` before any build/extract command.
DRILL-41e01316
<!-- PREFLIGHT-TOKEN:END -->
