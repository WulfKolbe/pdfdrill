...
The badge opens [**drillspace**](https://github.com/WulfKolbe/drillspace) — a
codespace where drillui comes up in your browser on a drilled sample paper.
Two clicks, nothing to install. To hack on pdfdrill itself, open *this* repo in
a container instead ([`.devcontainer/`](.devcontainer/README.md)).
...
| `pdfdrill beamer <pdf> [--compile]` | `latex/<bibkey>.beamer.tex` | **PROJECT the docmodel to a beamer slide deck** — one frame per Section (`allowframebreaks`), title + outline + References frames. Same source-agnostic projection as `latex` (transclusions, `\cite`, lists resolve), a deck instead of an article. Compile with **xelatex** |
