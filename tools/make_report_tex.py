...
def col_widths(usable_mm: float, with_image: bool):
    """(ident, src, rendered, [image,] page) widths in mm for the usable span.
    ...
    if with_image:
        ...
        return ident, src, rend, img, page
    src = round(rest / 2)
    return ident, src, rest - src, page


def table_open(caption: str, widths) -> str:
    cols = "|" + "|".join("p{%smm}" % w for w in widths) + "|"
    heads = {4: ("Identifier", "LaTeX source", "Rendered", "Page"),
             5: ("Identifier", "LaTeX source", "Rendered", "Scan image",
                 "Page")}[len(widths)]
    return (
        "\\section*{%s}\n" % caption +
        "\\begin{longtable}{%s}\n\\hline\n" % cols +
...
def main() -> None:
    ...
    img_col = eq_widths[3] if crops else None
