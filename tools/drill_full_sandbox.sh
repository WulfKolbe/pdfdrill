...
# SANDBOX: pdfdrill reads .env internally, but this script's guards test the
# SHELL environment. Load it so `if [ -n "${MATHPIX_APP_ID:-}" ]` can fire.
if [ -f "$REPO/.env" ]; then
...
if [ -n "${MATHPIX_APP_ID:-}" ]; then
  # SANDBOX: --force re-uploads and re-charges. Only force when nothing cached.
  if ls "${DOC%.pdf}".lines.json >/dev/null 2>&1; then
    step "mathpix (cached — no re-upload)" "${PD[@]}" mathpix "$DOC"
  else
    step "mathpix (paid; --force)" "${PD[@]}" mathpix "$DOC" --force
  fi
else
