...
## Before you add an ELEVENTH latex field

There are already ten, because `latex` means three things at once — the
MathPix reading, macro-expanded, un-refined — and every new combination has
been answered with a new sibling name that nothing composes with. The
decision (user, 2026-09-14) is PROMOTION: the best available reading gets
the standard name (`latex`, or `latex_code` for Diagram/LtxCommand/Table),
every other reading keeps a copy under a source-named field, and the WRITER
promotes so no reader needs a flag. pdfminer LaTeX arrives shortly and is
authoritative on symbol names.

Four constraints, each measured, are in `docmodel.prop_contract`'s module
docstring and in full in
`docs/superpowers/specs/2026-09-14-latex-field-promotion-design.md`. The
shortest one: `latex_raw` is 112,066 objects with no reader, kept so a
normalisation defect would be recoverable, and nothing has ever recovered
one. A copy no reader is named for is not an audit trail, it is weight.

## Every prop

119 props over 26 object types, from 1350 model.docmodel.json under ~/pdfdrill-library.
...
| `first_section_id` | 1,346 | Document | pdfdrill model | **—** summary: written on the Document object; consumers walk the flow instead. (mentioned in 3 files) |
| `total_pages` | 1,346 | Document | pdfdrill model | **—** summary: a count written on the Document object; consumers walk the flow and count for themselves. (mentioned in 5 files) |
| `total_paragraphs` | 1,346 | Document | pdfdrill model | **—** summary: a Paragraph count on the Document object; consumers count the objects instead. (mentioned in 2 files) |
| `total_sections` | 1,346 | Document | pdfdrill model | **—** summary: a Section count on the Document object; consumers count the objects instead. (mentioned in 2 files) |
...
| `context` | 254 | Link | pdfdrill links | **—** GAP: the prose surrounding a Link, 254 objects. Recorded when the link was found and never read back. (mentioned in 6 files) |
| `dest_name` | 254 | Link | pdfdrill links | `pdfdrill/annotations.py` |
| `dest_page` | 254 | Link | pdfdrill links | `pdfdrill/annotations.py` |
| `uri` | 254 | Link | pdfdrill links | **—** GAP: the Link's target. The `links` command reads the annotation layer directly rather than these objects. (mentioned in 4 files) |
...
| `source_object` | 51 | MathTail | — | **—** provenance: the object a derived value came from. (mentioned in 2 files) |
| `latex_pretail` | 47 | Equation, Formula | — | `pdfdrill/commands.py` |
| `latex_overlaid` | 44 | Table | — | `pdfdrill/commands.py` |
| `page_before_repair` | 34 | Section | pdfdrill clean | **—** provenance: a Section's page before heading_cleanup moved it. Written with setdefault, so the scan sees no reader. (mentioned in 2 files) |
| `latex_refined` | 31 | Equation, Formula | — | **—** READ THROUGH A CONSTANT: refine.REFINED_FIELD. The `--prefer-refined` projection reads it (233); a name-literal scan cannot see that. (mentioned in 5 files) |
| `indent_norm` | 21 | List | — | **—** GAP: normalised ListItem indentation, 21 objects. (mentioned in 3 files) |
| `list_type` | 21 | List | — | **—** GAP: ordered vs unordered, 21 objects. No projector distinguishes them. (mentioned in 2 files) |
